...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
// create-lambda-app is installed globally on people's computers. This means
// that it is extremely difficult to have them upgrade the version and
// because there's only one global version installed, it is very prone to
// breaking changes.
//
// The only job of create-lambda-app is to init the repository and then
// forward all the commands to the local version of create-lambda-app.
//
// If you need to add a new command, please add it to the scripts/ folder.
//
// The only reason to modify this file is to add more warnings and
// troubleshooting information for the `create-lambda-app` command.
//
// Do not make breaking changes! We absolutely don't want to have to
// tell people to update their global version of create-lambda-app.
//
...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    ...
    'use a non-standard version of lambda-scripts'
  )
  .allowUnknownOption()
  .on('--help', () => {
    ...
    console.log(
      `      - a custom fork published on npm: ${chalk.green('my-lambda-scripts')}`
    );
    console.log(
      `      - a .tgz archive: ${chalk.green('https://mysite.com/my-lambda-scripts-0.8.2.tgz')}`
    );
    console.log(
    ...
    console.log(
      `      ${chalk.cyan('https://github.com/channl/create-lambda-app/issues/new')}`
    );
    console.log();
  })
  .parse(process.argv);
...
if (typeof projectName === 'undefined') {
  ...
  console.log(`  ${chalk.cyan(program.name())} ${chalk.green('my-lambda-app')}`);
  console.log();
  console.log(
...
function createApp(name, verbose, version, template) {
  ...
  console.log(`Creating a new Lambda app in ${chalk.green(root)}.`);
  console.log();
...
function run(root, appName, version, verbose, originalDirectory, template) {
  ...
  getPackageName(packageToInstall)
    ...
    .then(packageName => {
      ...
      // Since lambda-scripts has been installed with --save
      // we need to move it into devDependencies and rewrite package.json
      // also ensure lambda dependencies have caret version range
...
function getInstallPackage(version) {
  let packageToInstall = 'lambda-scripts';
...
function getPackageName(installPackage) {
  if (installPackage.indexOf('.tgz') > -1) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. lambda-scripts-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/lambda-scripts.git
    // git+ssh://github.com/mycompany/lambda-scripts.git#v1.2.3
    ...
    console.error(
      chalk.red(
        'You are running Node %s.\n' +
          'Create Lambda App requires Node %s or higher. \n' +
...
function checkAppName(appName) {
  ...
  const dependencies = [];
  const devDependencies = ['lambda-scripts'];
